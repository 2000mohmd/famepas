...
WITH CHECK (bucket_id = 'brief-images' AND EXISTS (SELECT 1 FROM public.venues v WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(storage.objects.name))[1]));
CREATE POLICY "Venue owners can update own brief images" ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id = 'brief-images' AND EXISTS (SELECT 1 FROM public.venues v WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(storage.objects.name))[1]));
...
WITH CHECK (bucket_id = 'venue-photos' AND EXISTS (SELECT 1 FROM public.venues v WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(storage.objects.name))[1]));
CREATE POLICY "Owners update venue-photos" ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id = 'venue-photos' AND EXISTS (SELECT 1 FROM public.venues v WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(storage.objects.name))[1]));
...
WITH CHECK (bucket_id = 'offer-images' AND EXISTS (SELECT 1 FROM public.venues v WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(storage.objects.name))[1]));
CREATE POLICY "Venue owners can update own offer images" ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id = 'offer-images' AND EXISTS (SELECT 1 FROM public.venues v WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(storage.objects.name))[1]));
