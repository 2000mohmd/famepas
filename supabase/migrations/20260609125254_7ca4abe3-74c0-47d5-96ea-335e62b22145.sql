...
CREATE POLICY "Venue owners can upload offer images" ON storage.objects FOR INSERT
  WITH CHECK (bucket_id = 'offer-images' AND EXISTS (
    SELECT 1 FROM public.venues v
    WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(name))[1]
  ));
CREATE POLICY "Venue owners can update own offer images" ON storage.objects FOR UPDATE
  USING (bucket_id = 'offer-images' AND EXISTS (
    SELECT 1 FROM public.venues v
    WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(name))[1]
  ));
CREATE POLICY "Venue owners can delete own offer images" ON storage.objects FOR DELETE
  USING (bucket_id = 'offer-images' AND EXISTS (
    SELECT 1 FROM public.venues v
    WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(name))[1]
  ));

CREATE POLICY "Venue owners can upload brief images" ON storage.objects FOR INSERT
  WITH CHECK (bucket_id = 'brief-images' AND EXISTS (
    SELECT 1 FROM public.venues v
    WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(name))[1]
  ));
CREATE POLICY "Venue owners can update own brief images" ON storage.objects FOR UPDATE
  USING (bucket_id = 'brief-images' AND EXISTS (
    SELECT 1 FROM public.venues v
    WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(name))[1]
  ));
CREATE POLICY "Venue owners can delete own brief images" ON storage.objects FOR DELETE
  USING (bucket_id = 'brief-images' AND EXISTS (
    SELECT 1 FROM public.venues v
    WHERE v.owner_id = auth.uid() AND v.id::text = (storage.foldername(name))[1]
  ));

CREATE POLICY "Venue owners can view deliverables for their bookings" ON storage.objects FOR SELECT
