    ...
    <section className="relative min-h-[88vh] flex items-center overflow-hidden pt-20">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full py-16">
        <div className="max-w-2xl space-y-6">
          ...
          <p className="text-base sm:text-lg text-muted-foreground max-w-lg leading-relaxed">{slide.subtitle}</p>
          <div className="flex flex-wrap gap-3 pt-2">
            <Button size="lg" className="bg-accent text-accent-foreground hover:bg-accent/90 gap-2 px-8 rounded-xl shadow-lg shadow-accent/20" asChild>
              <Link to={slide.ctaLink}>
                <MapPin className="w-5 h-5" />
                {slide.cta}
              </Link>
            </Button>
            <Button size="lg" variant="outline" className="border-border hover:bg-card/80 px-8 rounded-xl" asChild>
              <Link to="/categories">Browse Categories</Link>
            </Button>
          </div>

          {/* Stats bar */}
          <div className="flex items-center gap-8 pt-6 border-t border-border/50 mt-2">
            {[
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3">
          <button onClick={() => setCurrent((p) => (p - 1 + slides.length) % slides.length)} className="p-2 rounded-full bg-card/60 border border-border hover:bg-card transition-colors backdrop-blur-sm">
          ...
          <button onClick={() => setCurrent((p) => (p + 1) % slides.length)} className="p-2 rounded-full bg-card/60 border border-border hover:bg-card transition-colors backdrop-blur-sm">
