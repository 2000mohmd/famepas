...
const VenuesPage = () => {
  ...
  const { data: categories } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("categories").select("id, owner_id, brand_id, name, description, category, address, city, country, latitude, longitude, website, logo_url, cover_image_url, is_active, approval_status, venue_type, created_at").eq("is_active", true);
      return data ?? [];
    },
  });
