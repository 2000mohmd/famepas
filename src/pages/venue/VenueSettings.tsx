...
const VenueSettings = () => {
  ...
  const load = async () => {
    ...
    const [sRes, tplRes, tierRes, catRes, invRes] = await Promise.all([
      supabase.from("social_integrations").select("id, venue_id, platform, handle, status, connected_at, token_expires_at, open_id, scope, avatar_url, display_name, created_at, updated_at").eq("venue_id", v.id),
